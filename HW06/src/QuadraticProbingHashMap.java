...
public class QuadraticProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("Illegal input! Either key or value was a null pointer.");
        }
        ...
        int lastDelMarker = -1;
        V oldVal = null;
        int index = ((int) Math.pow(quadProbInt, 2) + key.hashCode()) % table.length;
        boolean foundPos = false;
        while (!foundPos && quadProbInt < table.length) {
            if (table[index] == null) {
                if (lastDelMarker >= 0) {
                    table[lastDelMarker] = new QuadraticProbingMapEntry(key, value);
                } else {
                    table[index] = new QuadraticProbingMapEntry(key, value);
                }
                size++;
                foundPos = true;
            } else if (lastDelMarker == -1 && table[index].isRemoved()) {
                lastDelMarker = index;
            ...
            quadProbInt++;
            index = ((int) Math.pow(quadProbInt, 2) + key.hashCode()) % table.length;
        }
        if (!foundPos && quadProbInt == table.length) {
            resizeBackingTable((table.length * 2) + 1);
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Input key was a null pointer!");
        }
        V returnVal = null;
        int quadProbInt = 0;
        int index = ((int) Math.pow(quadProbInt, 2) + key.hashCode()) % table.length;
        boolean foundPos = false;
        while (!foundPos && quadProbInt < table.length) {
            if (table[index] == null || quadProbInt == table.length - 1 || table[index].isRemoved()) {
                throw new NoSuchElementException("Key doesn't exist in the map!");
            } else if (table[index].getKey().equals(key)) {
    ...
    public boolean containsKey(K key) {
        ...
        while (!foundPos && quadProbInt < table.length) {
            if (table[index] == null) {
                break;
            } else if (table[index].isRemoved() && lastDelMarker == -1) {
                lastDelMarker = index;
            } else if (table[index].getKey().equals(key)) {
                foundPos = true;
            }
     ...
     */
    public Set<K> keySet() {
        HashSet<K> keyList = new HashSet<>(size);
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("Input length is less than number of items present!");
        }
        ...
        for (int i = 0; i < table.length; i++) {
            if (table[i] != null && !table[i].isRemoved()) {
                ...
                while (newTable[newIndex] != null) {
                    quadProbInt++;
                    newIndex = (newIndex + (int) Math.pow(quadProbInt, 2)) % newTable.length;
                }
                newTable[newIndex] = new QuadraticProbingMapEntry<>(table[i].getKey(), table[i].getValue());
            }
        }
        table = newTable;
    }
