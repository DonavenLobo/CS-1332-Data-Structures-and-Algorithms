...
public class PatternMatching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text,
        ...
        if (pattern == null || pattern.length() == 0 || text == null || comparator == null) {
            throw new IllegalArgumentException("An input argument was insufficient for KMP pattern matching.");
        }
        ...
        while (textIndex <= text.length() - pattern.length()) {
            while (patternIndex < pattern.length() && comparator.compare(text.charAt(textIndex + patternIndex),
            ...
            if (patternIndex == 0) {
                ...
                int shift = failureTable[patternIndex - 1];
    ...
    public static int[] buildFailureTable(CharSequence pattern,
        ...
        if (pattern == null || comparator == null) {
            throw new IllegalArgumentException("One of the given inputs was a null argument");
        }
        if (pattern.length() == 0) {
            return new int[0];
        }
    ...
    public static List<Integer> boyerMoore(CharSequence pattern, CharSequence text, CharacterComparator comparator) {
        if (pattern == null || pattern.length() == 0 || text == null || comparator == null) {
            throw new IllegalArgumentException("An input argument was insufficient for pattern matching");
        }
        if (pattern.length() > text.length()) {
            return new ArrayList<Integer>();
        }
    ...
    public static Map<Character, Integer> buildLastTable(CharSequence pattern) {
        if (pattern == null) {
            throw new IllegalArgumentException("Input pattern was a null value!");
        }
        int m = pattern.length();
    ...
    public static List<Integer> rabinKarp(CharSequence pattern, CharSequence text, CharacterComparator comparator) {
        if (pattern == null || pattern.length() == 0 || text == null || comparator == null) {
            throw new IllegalArgumentException("An input argument was insufficient for Rabin Karp pattern matching.");
        }
        if (pattern.length() > text.length()) {
            return new ArrayList<Integer>();
        }
        ...
        for (int k = 0; k <= text.length() - pattern.length(); k++) {
            ...
            if (k < text.length() - pattern.length()) {
                textHash = (textHash - (text.charAt(k) * lastBase)) * BASE + text.charAt(k + pattern.length());
            }
        }
        return matches;
    }
